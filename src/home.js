...
function createCard(product) {
    ...
    for (let i = 0; i < pcItems.length; i++) {
        ...
        priceValue.setAttribute('id', 'price-value');
        priceValue.textContent = pcItems[i].prezzo;
        var button = document.createElement('button');
        button.classList.add('add-button');
        button.addEventListener('click', () => add(pcItems[i].id));
        ...
        if (pcItems[i].promozione){
           ...
           priceValue.classList.add('delete-price')
           
          }
          prezzo.appendChild(priceValue);
          ...
          if (pcItems[i].promozione){
            var promozione = document.createElement('p');
            promozione.textContent = pcItems[i].sconto;
            promozione.classList.add('discount');
            cardContent.appendChild(promozione);
...
function getCartModal(cartItems) {
  ...
  var pcItems = portatili.concat(fissi,smartphone);
  
  // portatili.concat(fissi).concat(smartphone);
  ...
  cartItems.forEach((v) => {
    ...
    const item = pcItems.find((it) => it.id === id);
    ...
    gradeValue.textContent = item.grado;
    ...
    if (item?.promozione && item?.sconto) {
      priceValue.textContent = item?.sconto;

    } else {
      priceValue.textContent = item?.prezzo;
    }
